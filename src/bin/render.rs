...
//!
//! The three toggles are the point:
//!
...
//! * **C** turns off backface culling — the far side of every object starts
//!   fighting the near side.

use minifb::{Key, KeyRepeat, Window, WindowOptions};
use recursion1::quat::Q;
use recursion1::raster::{colour, Canvas};
use recursion1::render3::{Camera, Light, Mesh, Renderer, Vert};
...
fn main() {
    ...
    if std::env::args().any(|a| a == "--snapshot") {
        ...
        render(&mut c, &mut r, &cam, &objs, &light, 1.1);
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        let cam = camera(orbit, elev, dist);
        render(&mut canvas, &mut r, &cam, &objs, &light, t);
...
fn render(c: &mut Canvas, r: &mut Renderer, cam: &Camera, objs: &[Obj], light: &Light, t: f64) {
    c.clear(colour::BG);
    r.begin();
    for o in objs {
        ...
        for tri in &o.mesh.tris {
            let place = |v: &Vert| Vert {
                pos: q.rotate(v.pos) + o.at,
                normal: q.rotate(v.normal),
                uv: v.uv,
            };
            r.triangle(c, cam, [place(&tri[0]), place(&tri[1]), place(&tri[2])], o.colour, light, o.checker);
        }
    }
}
...
fn hud(c: &mut Canvas, r: &Renderer, scene: &str, fps: f64, paused: bool) {
    ...
    let help = "ARROWS ORBIT  W/S ZOOM  P PERSPECTIVE  Z DEPTH  C CULL  TAB SCENE  SPACE PAUSE";
