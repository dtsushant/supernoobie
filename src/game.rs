...
/// World to screen. World `y` points up, screen rows count down — the same
/// flip every 2-D demo in this crate makes, in one place at last.
#[derive(Clone, Copy)]
pub struct View {
    pub w: f64,
    pub h: f64,
...
impl View {
    pub fn new(w: usize, h: usize) -> Self {
        View { w: w as f64, h: h as f64 }
    }
    pub fn to_screen(&self, p: Cx) -> (i32, i32) {
        (p.re as i32, (self.h - p.im) as i32)
    }
    ...
    pub fn ring(&self, c: &mut Canvas, at: Cx, r: f64, t: i32, col: u32) {
        ...
        c.ring(p.0, p.1, r as i32, t, col);
    }
    pub fn disc(&self, c: &mut Canvas, at: Cx, r: f64, col: u32) {
        let p = self.to_screen(at);
        c.disc(p.0, p.1, r as i32, col);
    }
...
pub mod pen {
    ...
    pub fn arrow(c: &mut Canvas, v: &View, from: Cx, to: Cx, col: u32, label: Option<&str>) {
        ...
        if let Some(t) = label {
            v.text(c, to + u.scale(12.0), t, col, 1);
        }
    }
    ...
    pub fn dimension(c: &mut Canvas, v: &View, a: Cx, b: Cx, offset: f64, label: &str) {
        ...
        let t = (d.unit() * Cx::expi(0.785)).scale(6.0);
        v.line(c, a2 - t, a2 + t, 1, DIM);
        v.line(c, b2 - t, b2 + t, 1, DIM);
        let mid = (a2 + b2).scale(0.5) + (Cx::I * d.unit()).scale(8.0);
    ...
    pub fn angle_arc(c: &mut Canvas, v: &View, centre: Cx, r: f64, a0: f64, a1: f64, label: &str) {
        ...
        let mid = centre + Cx::expi((a0 + a1) * 0.5).scale(r + 16.0);
    ...
    pub fn radius(c: &mut Canvas, v: &View, centre: Cx, r: f64, at_angle: f64, label: &str) {
        ...
        v.disc(c, centre, 2.0, RAD);
        let mid = centre + Cx::expi(at_angle).scale(r * 0.55);
        v.text_mid(c, mid + Cx::new(0.0, 9.0), label, RAD, 1);
    }
